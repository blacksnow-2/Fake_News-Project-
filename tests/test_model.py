...
def test_rf_overfits_small_dataset(config, sample_datapoints):
    ...
    # This part uses the map() function to apply a lambda function to each element in predicted_labels. 
    # The lambda function lambda x: bool(x) converts each integer value x to a boolean value using the bool() function. 
...
def test_rf_correct_predict_range(config, sample_datapoints):
    ...
    # <= 1 retruns an array of tru and false. all() checks if all are true or not.
