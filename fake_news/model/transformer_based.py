...
# All hooks https://lightning.ai/docs/pytorch/stable/_modules/lightning/pytorch/core/hooks.html#ModelHooks.on_validation_epoch_end

# Search for pl.Lightning Module
class RobertaModule(pl.LightningModule):
    def __init__(self, config: Dict):
        # The super().__init__() call is used in Python to invoke the 
        # constructor of the superclass (or parent class) of the current class.
        super().__init__()
        base_dir = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))
        full_model_output_path = os.path.join(base_dir, config["model_output_path"])
        self.config = config
        self.classifier = RobertaForSequenceClassification.from_pretrained(config["type"], cache_dir=full_model_output_path)
        # to work on latest version
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.val_step_outputs.append(output[0])
        return output[0]
    
    # Updated to latest version
    def on_validation_epoch_end(self) -> None:
        avg_val_loss = float(sum(self.val_step_outputs) / len(self.val_step_outputs))
...
class RobertaModel(Model):
    def __init__(self, config: Dict, load_from_ckpt: bool = False):
        ...
        if load_from_ckpt:
            ...
            #                        logger=False)
            # Deprecated argument modified. auto is none if there is no gpu, or else.
            self.trainer = Trainer(max_epochs=self.config["num_epochs"],
                                   accelerator="auto",
                                   callbacks=[checkpoint_callback],
                                   logger=False)
